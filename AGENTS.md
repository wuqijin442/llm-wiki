...
>
> 版本：v2 ｜ 更新：2026-09-12 ｜ 相对 v1 变更：新增 §4 自生长机制、§5 冲突与不确定性处理、§6 页面生命周期；§2 增补 `status`/`confidence`/`scope` 可选字段；Ingest / Lint 增补增量检测与熵减步骤。
...
**生长检查点**（建议按周执行）：打开 `wiki/growth.md` 看四项指标——未消化素材数、孤立页数、缺 `description` 页数、超 90 天未更新页数。任一项 > 0 即安排一次 `ingest` 或 `lint`。
...
0. **差异检测**：先列出 `raw/` 中未被任何 wiki 页引用的素材，明确本次消化范围（单篇 or 批量）
1. 读取素材全文，提取关键信息
2. 向用户汇报关键发现与拟建页面清单，确认重点
