...
| 动作 | 检查点 |
| --- | --- |
| ① 差异检测 | 明确本次要消化哪些素材（而非无目标地遍历全部） |
| ② 增量归并 | **先查已有页再决定新建**——命中就补充，不重复造页 |
| ③ 交叉链接 | 新页 ≥2 出链、≥1 入链，且登记进 `index.md` |
| ④ 留痕 | 追加 `log.md`；刷新 `updated` 与 `status`；缺口写入「待核实」 |

**熵减也算生长**：合并重复页、修正错误、补全缺失字段、清理断链、刷新 `stale` 页，与新增页面同等重要。指标只增不减不是目标。
